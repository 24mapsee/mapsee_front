...
class FollowingFollowerPage extends StatefulWidget {
  ...
  const FollowingFollowerPage(
      {super.key, required this.initialTabIndex, required this.accessUserId});
...
class _FollowingFollowerPageState extends State<FollowingFollowerPage>
  ...
  Future<void> fetchFollowerAndFollowingData() async {
    final followerUrl =
        '${dotenv.env["API_BASE_URL"]}/follow/follower/${widget.accessUserId}';
    final followingUrl =
        '${dotenv.env["API_BASE_URL"]}/follow/following/${widget.accessUserId}';
    ...
    try {
      ...
      if (followerResponse.statusCode == 200 &&
        ...
        setState(() {
          ...
          followerData =
              List<Map<String, dynamic>>.from(followerJson['followers']);
          followingData =
              List<Map<String, dynamic>>.from(followingJson['following']);
          isLoading = false;
        });
            ...
            child: TabBarView(
              ...
              children: [
                ...
                _buildListView(
                    ...
                    (loginUserId == widget.accessUserId)
                        ? deleteFollower
                        : doFollowing),
              ],
            ),
...
Widget _buildListView(BuildContext context, List<Map<String, dynamic>> data,
    String buttonText, Future<void> Function(String) onButtonPressed) {
  // 1. onButtonPressed 타입 수정
  ...
  return ListView.builder(
    ...
    itemBuilder: (context, index) {
      ...
      final profileImg = data[index]['profile_picture'] ?? '';
      ...
      return ListTile(
        leading: profileImg.startsWith('http')
            ? Image.network(profileImg,
                width: 40, height: 40) // URL인 경우 Image.network 사용
            : Image.asset(profileImg,
                width: 40, height: 40), // 로컬 파일인 경우 Image.asset 사용
